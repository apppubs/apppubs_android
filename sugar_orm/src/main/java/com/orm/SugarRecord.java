...
public abstract class SugarRecord {
    ...
    public static <T> T findByProperty(Class<T> type,String property,String value){
    	 List<T> list = find(type, property+"=?", new String[]{value}, null, null, "1");
     ...
     * @return
     */
    public static <T> int sumColumn(Class<T> type,String column){
    	int size = 0;
    	 SugarDb db = getSugarContext().getSugarDb();
         SQLiteDatabase sqLiteDatabase = db.getDB();
         Cursor cur = sqLiteDatabase.rawQuery("select sum("+column+") from "+NamingHelper.toSQLName(type),null);
    ...
    public static <T> List<T> find(Class<T> type, String whereClause, String  whereArgs) {
    	return find(type, whereClause, new String[]{whereArgs}, null, null, null);
    }
     ...
     * @param value
     */
    public static void updateById(Class<?> type ,String id,String property,String value){
    	 SugarDb db = getSugarContext().getSugarDb();
         SQLiteDatabase sqLiteDatabase = db.getDB();
         ContentValues cvs = new ContentValues();
         cvs.put(property, value);
         sqLiteDatabase.update(NamingHelper.toSQLName(type), cvs, "ID=?", new String[]{id});
     ...
     * @param values
     */
    public static void updateById(Class<?> type,String id,String[] propertys , String[] values){
    	 SugarDb db = getSugarContext().getSugarDb();
         SQLiteDatabase sqLiteDatabase = db.getDB();
         ContentValues cvs = new ContentValues();
         int size = propertys.length;
         for(int i=-1;++i<size;){
        	 cvs.put(propertys[i], values[i]);
         }
         sqLiteDatabase.update(NamingHelper.toSQLName(type), cvs, "ID=?", new String[]{id});
    }
    /**
     * 更新某个字段，通过某个条件(可以唯一确定一条记录的条件)
     * @param type
     * @param property
     * @param value
     * @param whereProperty
     * @param conditonV
     */
    public static void update(Class<?> type,String property,String value,String whereClause,String[] conditon){
    	 SugarDb db = getSugarContext().getSugarDb();
         SQLiteDatabase sqLiteDatabase = db.getDB();
         ContentValues cvs = new ContentValues();
         cvs.put(property, value);
         sqLiteDatabase.update(NamingHelper.toSQLName(type), cvs, whereClause, conditon);
    }
...
//        Log.i("Sugar", object.getClass().getSimpleName() + " saved : "+id);
    ...
    private static void inflate1(Cursor cursor,Object object){
        List<Field> columns = ReflectionUtil.getTableFields(object.getClass());
