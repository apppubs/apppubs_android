...
public class DragImageView extends ImageView {
	...
	private boolean isLeftEdgeVisiable;//左边缘是否可见
	private boolean isRightEdgeVisiable;//有边缘是否可见
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if(isScaled()&&!isLeftEdgeVisiable&&!isRightEdgeVisiable){
		...
		if (mode == MODE.DRAG) {
			...
			if (isControl_H) {
				if (left >= 0) {
					left = 0;
					right = this.getWidth();
					isLeftEdgeVisiable = true;
				}else{
					isLeftEdgeVisiable = false;
				}
				if (right <= screen_W) {
					left = screen_W - this.getWidth();
					right = screen_W;
					isRightEdgeVisiable = true;
				}else{
					isRightEdgeVisiable = false;
				}
			} else {
				left = this.getLeft();
				right = this.getRight();
	...
	public boolean isRightEdgeVisiable() {
		return isRightEdgeVisiable;
	}

	public boolean isLeftEdgeVisiable() {
		return isLeftEdgeVisiable;
	}
}
