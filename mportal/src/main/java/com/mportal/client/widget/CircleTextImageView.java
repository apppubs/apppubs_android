...
public class CircleTextImageView extends ImageView {
    ...
    private int mTextMaxLength = DEFAULT_TEXT_MAX_LENGTH;
    ...
    public CircleTextImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mTextMaxLength = (int)a.getDimension(R.styleable.CircleTextImageView_citv_text_max_length,DEFAULT_TEXT_MAX_LENGTH);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mFillColor != Color.TRANSPARENT) {
            canvas.drawRoundRect(rect,(float)(getWidth()*0.1),(float)(getWidth()*0.1),mFillPaint);
        }
        if(mBitmap!=null) {
            canvas.drawRoundRect(rect,(float)(getWidth()*0.1),(float)(getWidth()*0.1),mBitmapPaint);
        }
        if (mBorderWidth != 0) {
            canvas.drawRoundRect(rect,(float)(getWidth()*0.1),(float)(getWidth()*0.1),mBorderPaint);
        }

        if (!TextUtils.isEmpty(mTextString)&&mBitmap==null) {
            Paint.FontMetricsInt fm = mTextPaint.getFontMetricsInt();
            mTextString = mTextString.length()>mTextMaxLength?mTextString.substring(0,mTextMaxLength):mTextString;
            float width = mTextPaint.measureText(mTextString);
